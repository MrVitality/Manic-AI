 ...
 * On Windows, rename over an existing file requires unlinking first.
 */
...
/** Try to restore a backup file; log result either way. */
function _restoreBackup(backupPath, targetPath) {
...
function saveAliases(aliases) {
  ...
  try {
    ...
    _atomicWrite(aliasesPath, JSON.stringify(aliases, null, 2));
    try { if (fs.existsSync(backupPath)) fs.unlinkSync(backupPath); } catch { /* non-critical */ }
    return true;
  } catch (err) {
    log(`[Aliases] Error saving aliases: ${err.message}`);
    _restoreBackup(backupPath, aliasesPath);
    try { if (fs.existsSync(aliasesPath + '.tmp')) fs.unlinkSync(aliasesPath + '.tmp'); } catch { /* non-critical */ }
    return false;
  }
}
