...
/**
 * Recursively collect files matching regex from a directory.
 * Internal helper for findFiles.
 */
function _collectMatchingFiles(currentDir, regex, maxAge, recursive, results) {
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile() && regex.test(entry.name)) {
        let stats;
        try { stats = fs.statSync(fullPath); } catch { continue; } // deleted between readdir and stat
        const ageInDays = (Date.now() - stats.mtimeMs) / (1000 * 60 * 60 * 24);
        if (maxAge === null || ageInDays <= maxAge) {
          results.push({ path: fullPath, mtime: stats.mtimeMs });
        }
      } else if (entry.isDirectory() && recursive) {
        _collectMatchingFiles(fullPath, regex, maxAge, recursive, results);
      }
    }
  } catch {
...
function findFiles(dir, pattern, options = {}) {
  ...
  const results = [];
  _collectMatchingFiles(dir, regex, maxAge, recursive, results);
  results.sort((a, b) => b.mtime - a.mtime);
  return results;
}

/** Parse buffered stdin data as JSON, returning {} on empty or malformed input. */
function _parseStdinBuffer(data) {
  try { return data.trim() ? JSON.parse(data) : {}; } catch { return {}; }
}

/** Remove all stdin listeners and unref so the event loop can exit. */
function _cleanupStdin() {
...
async function readStdinJson(options = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      _cleanupStdin();
      settle(_parseStdinBuffer(data));
    }, timeoutMs);

    process.stdin.setEncoding('utf8');
    process.stdin.on('data', chunk => { if (data.length < maxSize) data += chunk; });
    process.stdin.on('end', () => { clearTimeout(timer); settle(_parseStdinBuffer(data)); });
    process.stdin.on('error', () => { clearTimeout(timer); settle({}); });
  });
}
...
/**
 * Output to stdout (returned to Claude via hook protocol).
 * Uses console.log intentionally — hooks communicate via stdout.
 * The scripts/ directory is excluded from the console.log lint check.
 */
