    ...
    <div className="flex-1 overflow-y-auto p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">Documents</h2>
            <p className="text-sm text-gray-400 mt-1">Upload and manage documents for RAG-enhanced conversations</p>
          </div>
          <button onClick={loadDocuments} className="px-3 py-1.5 text-sm glass-card hover:bg-white/10 transition-colors rounded-lg flex items-center gap-2">
        ...
        >
          ...
          <svg className="w-12 h-12 mx-auto mb-3 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
          <p className="font-medium">{uploading ? 'Uploading...' : isDragging ? 'Drop files here' : 'Drag & drop files or click to browse'}</p>
          <p className="text-sm text-gray-500 mt-1">Supports .txt, .md, .csv, .json, .html, .xml</p>
        </div>
        ...
        {isLoadingDocuments ? (
          ...
          <div className="text-center py-12">
            <svg className="w-16 h-16 mx-auto mb-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
            <p className="text-gray-400 font-medium">No documents yet</p>
            <p className="text-sm text-gray-500 mt-1">Upload documents to enhance your AI conversations with context</p>
          </div>
        ) : (
              ...
              <div key={doc.id} className="glass-card p-4 rounded-xl flex items-center gap-4 group">
                ...
                {deleteConfirm === doc.id ? (
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => setDeleteConfirm(null)} className="px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded transition-colors">No</button>
                  </div>
                ) : (
