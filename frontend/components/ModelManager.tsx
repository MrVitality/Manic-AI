    ...
    <div className="flex-1 overflow-y-auto p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">Models</h2>
            <p className="text-sm text-gray-400 mt-1">Manage your Ollama models</p>
          </div>
          <button onClick={refreshModels} disabled={isLoadingModels} className="px-3 py-1.5 text-sm glass-card hover:bg-white/10 transition-colors rounded-lg flex items-center gap-2">
        ...
        <div className="glass-card p-4 rounded-xl mb-6">
          <h3 className="text-sm font-medium text-gray-300 mb-3">Pull a New Model</h3>
          <div className="flex gap-3">
            ...
            />
            <button onClick={handlePull} disabled={isPulling || !pullName.trim()} className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 rounded-lg font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        {isLoadingModels ? (
          ...
          <div className="text-center py-12">
            <svg className="w-16 h-16 mx-auto mb-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
            <p className="text-gray-400 font-medium">No models installed</p>
            <p className="text-sm text-gray-500 mt-1">Pull a model above to get started</p>
          </div>
        ) : (
              ...
              <div key={model.name} className="glass-card p-4 rounded-xl flex items-center gap-4 group">
                ...
                {deleteConfirm === model.name ? (
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => setDeleteConfirm(null)} className="px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded transition-colors">Cancel</button>
                  </div>
                ) : (
